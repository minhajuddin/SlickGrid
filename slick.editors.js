...
(function($) {
    ...
    var SlickEditor = {
        ...
        DropDownCellFormatter : function(row, cell, value, columnDef, dataContext) {
            return columnDef.dataHash[value];
        },
        ...
        DropDownCellEditor : function(args) {
            ...
            this.init = function() {
              var optionsHash = args.column.dataHash;
              var options = [];
              for (key in optionsHash){
                options.push('<option value="'+ key +'">'+ optionsHash[key] +'</option>');
              }
              selectHtml = "<select class='drop-down-cell-select'>" + options.join() + "</select>";
              options = null;
              $input = $(selectHtml)
                      .appendTo(args.container)
                      .bind("keydown.nav", function(e) {
